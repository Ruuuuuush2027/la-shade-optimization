...
# =========================================================================
# SECTION 8B: VACANT SITE CANDIDATE FILTER
# =========================================================================
print("\n" + "=" * 80)
print("SECTION 8B: VACANT SITE CANDIDATE FILTER")
print("=" * 80)

VACANT_DISTANCE_THRESHOLD = 0.1  # kilometers; keep rows near plantable sites
vacant_distance_cols = ['dist_to_vacant_park_1', 'dist_to_vacant_street_1']
missing_vacant_cols = [col for col in vacant_distance_cols if col not in df_imputed.columns]

if missing_vacant_cols:
    print(f"Required distance columns missing: {missing_vacant_cols} - skipping candidate filter.")
else:
    initial_rows = len(df_imputed)
    candidate_mask = (
        (df_imputed['dist_to_vacant_park_1'].fillna(np.inf) < VACANT_DISTANCE_THRESHOLD) |
        (df_imputed['dist_to_vacant_street_1'].fillna(np.inf) < VACANT_DISTANCE_THRESHOLD)
    )
    df_imputed = df_imputed[candidate_mask].reset_index(drop=True)
    retained_rows = len(df_imputed)
    retention_pct = (retained_rows / initial_rows * 100) if initial_rows else 0
    print(f"Vacant-distance threshold: {VACANT_DISTANCE_THRESHOLD:.2f} km")
    print(f"Rows retained: {retained_rows}/{initial_rows} ({retention_pct:.2f}%)")
