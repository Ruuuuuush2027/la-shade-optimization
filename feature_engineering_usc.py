...
VACANT_DISTANCE_THRESHOLD = 0.1  # kilometers; only keep rows near plantable sites
...
# Filter to candidate sites before engineering features
missing_vacant_cols = [col for col in VACANT_DISTANCE_COLS if col not in df.columns]
if missing_vacant_cols:
    print(f"Warning: Missing vacant-distance columns {missing_vacant_cols}; skipping candidate filter.")
else:
    initial_rows = len(df)
    candidate_mask = (
        (df['dist_to_vacant_park_1'].fillna(np.inf) < VACANT_DISTANCE_THRESHOLD) |
        (df['dist_to_vacant_street_1'].fillna(np.inf) < VACANT_DISTANCE_THRESHOLD)
    )
    df = df[candidate_mask].reset_index(drop=True)
    retained_rows = len(df)
    retention_pct = (retained_rows / initial_rows * 100) if initial_rows else 0
    print(f"Applied vacant-distance filter ({VACANT_DISTANCE_THRESHOLD:.2f} km).")
    print(f"Candidate rows retained: {retained_rows}/{initial_rows} ({retention_pct:.2f}%)")
